        ...
        {items.length ? (
          items.map((entry, index) => (
            <div key={`${entry.timestamp}-${entry.agent}-${index}`} className="flex rounded-lg px-2 py-3 hover:bg-gray-50">
              <div className="mr-3 mt-1 h-9 w-9 shrink-0 rounded-full bg-ado-500 text-white">
              ...
              <div className="min-w-0 grow border-b border-gray-100 pb-3 text-sm last:border-b-0">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-gray-500">
                  ...
                  <span>Story</span>
                  <Link to={`/story/${entry.workItemId}`} className="font-semibold text-ado-500 hover:text-ado-600">
                    #{entry.workItemId}
                  </Link>
                </div>
              </div>
