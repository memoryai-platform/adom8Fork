...
public sealed class GitHubCodebaseOnboardingService : ICodebaseOnboardingService
{
    ...
    private static string BuildOrchestrationContractMarkdown()
    {
        ...
        sb.AppendLine();
        sb.AppendLine("## InitializeCodebase Exception");
        sb.AppendLine();
        sb.AppendLine("InitializeCodebase stories are a special no-clone path:");
        sb.AppendLine("- Routed directly to Coding");
        sb.AppendLine("- Review/Testing/Documentation/Deployment queue stages may be skipped");
        sb.AppendLine("- Story is moved to Code Review after Copilot reconciliation");
        sb.AppendLine();
