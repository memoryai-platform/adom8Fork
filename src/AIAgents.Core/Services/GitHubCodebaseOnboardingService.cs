...
public sealed class GitHubCodebaseOnboardingService : ICodebaseOnboardingService
{
    ...
    {
        var sb = new StringBuilder();
        sb.AppendLine("# Orchestration Contract");
        sb.AppendLine();
        sb.AppendLine("> Authoritative default generated by ADOm8 onboarding. Repository owner may supersede this file intentionally.");
        sb.AppendLine();
        sb.AppendLine("## Stage Order");
        sb.AppendLine();
        sb.AppendLine("`Planning → Coding → Testing → Review → Documentation → Deployment`");
        sb.AppendLine();
        sb.AppendLine("## Deployment Ownership Boundary");
        sb.AppendLine();
        sb.AppendLine("- GitHub agent orchestrates Planning through Documentation.");
        sb.AppendLine("- GitHub agent must not perform deployment directly.");
        sb.AppendLine("- Azure deployment agent owns merge/deploy policy execution.");
        sb.AppendLine("- GitHub agent signals Deployment readiness via MCP stage update.");
        sb.AppendLine();
        sb.AppendLine("## Planning Readiness Gate (AI Minimum Review Score)");
        sb.AppendLine();
        sb.AppendLine("AI Minimum Review Score applies to Planning readiness, not the later code review stage.");
        sb.AppendLine("AI Autonomy Level may appear as numeric or text labels and must be treated equivalently: 1/Plan Only, 2/Code Only, 3/Review & Pause, 4/Auto-Merge, 5/Full Autonomy.");
        sb.AppendLine("Planning must:");
        sb.AppendLine("1. Assess story readiness across clarity, ambiguity, dependencies, and acceptance completeness.");
        sb.AppendLine("2. Generate a Story Readiness Score (0-100).");
        sb.AppendLine("3. Produce explicit blocker and question lists.");
        sb.AppendLine("4. If score is below threshold, move story to Needs Revision and post one consolidated comment with all blockers and questions.");
        sb.AppendLine("5. If score meets/exceeds minimum and Autonomy Level > 1, proceed and document assumptions/choices used to continue.");
        sb.AppendLine("6. If autonomy is Level 1 (Plan Only), perform a full deep story analysis first, do not implement code changes, and route to Needs Revision with one consolidated blocker/question list and a brief proposed plan (3-5 bullets).");
        sb.AppendLine("7. For Level 1 after deep analysis, include exact line `No further info needed.` before presenting the brief proposed plan when no blockers remain.");
        sb.AppendLine();
        sb.AppendLine("## Required ADO Update Protocol");
        sb.AppendLine();
        sb.AppendLine("Priority order:");
        sb.AppendLine("1. Job #1: keep Azure DevOps board/fields/state fully up to date.");
        sb.AppendLine("2. Job #2: perform planning/coding/review implementation work.");
        sb.AppendLine();
        sb.AppendLine("At each completed stage, update Azure DevOps with all of:");
        sb.AppendLine("1. Work item state update when stage boundary requires it");
        sb.AppendLine("2. Work item comment with concrete evidence (plan, PR, tests, review outcome, deployment)");
        sb.AppendLine("3. Stage event record for traceability");
        sb.AppendLine("4. Current AI Agent and Last Agent fields kept in sync for stage visibility");
        sb.AppendLine("5. Completion protocol: No task is complete until you set ADO fields/state and add completion comment, then re-read and print final values.");
        sb.AppendLine("6. Completion comments should include: Outcome, ADO Updates Applied, Evidence, Final ADO Values.");
        sb.AppendLine();
        sb.AppendLine("When MCP bridge endpoints are configured, use:");
        sb.AppendLine("- `set-stage`");
        sb.AppendLine("- `add-comment`");
        sb.AppendLine("- `stage-event`");
        sb.AppendLine();
        sb.AppendLine("## Review Stage Scope");
        sb.AppendLine();
        sb.AppendLine("Review stage handles code quality/security feedback resolution.");
        sb.AppendLine("It does not own the Planning readiness threshold decision.");
        sb.AppendLine();
        sb.AppendLine("## Copilot Completion Gate Before Deployment Handoff");
        sb.AppendLine();
        sb.AppendLine("Handoff to Deployment requires all:");
        sb.AppendLine("- PR is not draft (Ready for Review)");
        sb.AppendLine("- PR title is not [WIP]");
        sb.AppendLine("- At least one changed file exists");
        sb.AppendLine("- Documentation-stage MCP updates are posted");
        sb.AppendLine("- Deployment stage signal is emitted via MCP");
        sb.AppendLine();
        sb.AppendLine("## Idempotency Rules");
        sb.AppendLine();
        sb.AppendLine("- Stage event submissions must be replay-safe.");
        sb.AppendLine("- Re-running the same stage must not duplicate destructive actions.");
        sb.AppendLine("- PR linking and progress reporting calls should be safe to repeat.");
        sb.AppendLine();
        sb.AppendLine("## PR and Branch Expectations");
        sb.AppendLine();
        sb.AppendLine("- Start from the story base branch provided in the issue body.");
        sb.AppendLine("- Open one PR per story task.");
        sb.AppendLine("- Include work item ID (`US-{id}`) in PR metadata for reconciliation.");
        sb.AppendLine();
        sb.AppendLine("## Security Guardrails");
        sb.AppendLine();
        sb.AppendLine("- Never commit secrets or credentials.");
        sb.AppendLine("- Follow repository-specific security rules in `AGENTS.md` when present.");
        sb.AppendLine("- Escalate if a requested change conflicts with compliance or policy constraints.");
