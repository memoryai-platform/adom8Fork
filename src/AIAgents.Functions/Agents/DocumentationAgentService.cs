...
## Prompt Truncation Metadata
- Generated code truncated: {(codeTruncated ? "yes" : "no")}
- Generated code omitted chars: {omittedCodeChars}
- Codebase context truncated: {(contextTruncated ? "yes" : "no")}
- Codebase context omitted chars: {omittedContextChars}

Generate comprehensive documentation for these changes.";

    failureStage = "ai_complete_documentation";
...
*Generated by AI Agent Pipeline*";
        ...
        failureStage = "create_pull_request";
        ...
        catch (HttpRequestException ex) when (ex.StatusCode == HttpStatusCode.NotFound)
        {
            return AgentResult.Fail(
                ...
                $"Documentation agent hit HTTP 404 for WI-{task.WorkItemId}. Common causes: Documentation model/deployment not available for configured AI provider, or repository/branch/PR target mismatch. HTTP detail: {ex.Message}",
                ex);
        }
        catch (HttpRequestException ex) when (ex.StatusCode == HttpStatusCode.BadRequest)
        {
            return AgentResult.Fail(
                ErrorCategory.Data,
                $"Documentation agent request was rejected for WI-{task.WorkItemId} (HTTP 400) at stage '{failureStage}'. Repo provider={_repoProvider.GetType().Name}, sourceBranch='{sourceBranch}', targetBranch='{targetBranch}'. HTTP detail: {ex.Message}",
                ex);
        }
        catch (HttpRequestException ex)
        {
            return AgentResult.Fail(ErrorCategory.Transient, $"HTTP error in Documentation agent for WI-{task.WorkItemId} at stage '{failureStage}': {ex.Message}", ex);
        }
        catch (Exception ex)
        {
    ...
    private static (string Text, bool Truncated, int OmittedChars) TruncateForPrompt(string? input, int maxChars)
    {
        ...
        var omittedChars = input.Length - maxChars;
        var trimmed = input[..maxChars];
        var marker = $"\n\n[TRUNCATED {omittedChars} CHARS TO FIT MODEL INPUT LIMITS]";
        return (trimmed + marker, true, omittedChars);
    }
